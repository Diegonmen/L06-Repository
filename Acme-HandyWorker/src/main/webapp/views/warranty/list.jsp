...
<display:table pagesize="3" class="displaytag" keepStatus="true"
	name="list" requestURI="/warranty/list.do" id="row">
	...
	<display:column value="${row.applicableLaws}"
		title="${warrantyApplicableLaws}"></display:column>

	<display:column>
		<jstl:if test="${row.finalMode==false }">
			<a href="warranty/adminstrator/edit.do?warrantyId=${row.id}">${warrantyEdit}</a>
		</jstl:if>
	</display:column>
	<display:column>
		<jstl:if test="${row.finalMode==false }">
			<a href="warranty/administrator/delete.do?warrantyId=${row.id}">${warrantyDelete}</a>
		</jstl:if>
	</display:column>
